...
public class RoomItemManager {
    ...
    public void addItemToRoom(int id, int x, int y, double z, int rot, int state, BaseItem baseItem) {
            floorItems.put(id, new RoomItem(id, x, y, z, rot, state, room, baseItem));
            room.getGameMap().addItemToMap(floorItems.get(id));
            room.sendMessage(new SerializeFloorItemComposer(floorItems.get(id)));
            room.getRoomUserManager().updateUserStatusses();
    ...
    public void addItemsToRoom(int id, int x, int y, double z, int rot, int state, BaseItem baseItem) {
    	int newid = id + 1;
        floorItems.put(id, new RoomItem(newid, x, y, z, rot, state, room, baseItem));
        room.getGameMap().addItemToMap(floorItems.get(newid));
        room.sendMessage(new SerializeFloorItemComposer(floorItems.get(newid)));
        room.getRoomUserManager().updateUserStatusses();
    ...
    public void addTo(int id, int x, int y, int rotation) {
   	 	...
   	 	if (item != null) {
   	 		...
   	 		room.getRoomItemManager().addItemToRoom(id, x, y, item.getZ(), rotation, item.getState(), item.getBaseItem());
   	 	}
    }
    ...
    // PickUP -> furniaddInventory -> that -> Game 
